const contenedorDispositiivos = document.getElementById("contenedor-dispositivos")
const contenedorCuidadoPersonal = document.getElementById("contenedor-cuidadoP")
...
db.collection("amazon").doc("salud").collection("nada1").doc("dispositivos").collection("nada2").get().then((querySnapshot)=>{
    querySnapshot.forEach(doc=>{
        ver(doc,fragmento)
    })
    contenedorDispositiivos.appendChild(fragmento)
})
db.collection("amazon").doc("salud").collection("nada1").doc("cuidadoPersonal").collection("nada2").get().then((querySnapshot)=>{
    querySnapshot.forEach(doc=>{
        ver(doc,fragmento1)
    })
    contenedorCuidadoPersonal.appendChild(fragmento1)
})
